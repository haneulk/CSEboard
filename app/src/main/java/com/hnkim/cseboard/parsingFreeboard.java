...
class parsingFreeboard extends Thread {
    ...
    public parsingFreeboard( String url,  int nCount){
        purl = url;
        pnCount = nCount;
    }
    public void run() {
...
//            Log.d("ERROR_TRSIZE", TBODY.getAllElements(HTMLElementName.TR).size() + "");
                ...
                try {
                    ...
                    if (pnCount == 1) {
                        if (TYPE.equals("공지")) {
                            ...
                            FreeNoticeListViewAdapter.noticeData.add(new FreeNoticeListData(TYPE, TITLE, URL, WRITER, DATE, VIEW));
                        }
                        if (!TYPE.equals("공지")) {
                            Element TR_INFO, TR_INFO_A, TR_WRITER, TR_DATE, TR_VIEW;
                            String TITLE, URL, WRITER, DATE, VIEW;
                            ...
                            FreeNoticeListViewAdapter.noticeData.add(new FreeNoticeListData(TYPE, TITLE, URL, WRITER, DATE, VIEW));
                            FreeNumberListViewAdapter.numberData.add(new FreeNumberListData(TYPE, TITLE, URL, WRITER, DATE, VIEW));
                        }
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    try {
                        MainFreeboard.FreeNoticeAdapter.notifyDataSetChanged(); //list 갱신
                        MainFreeboard.FreeNumberAdapter.notifyDataSetChanged();
                        MainFreeboard.fastScrollView3.listItemsChanged();
                        MainFreeboard.fastScrollView4.listItemsChanged();
                    } catch (Exception e) {
