...
public class ServiceNotice extends Service {
    ...
    public void onCreate() {
        if(SettingsActivity.select_noticeRefresh == null) {
        Toast.makeText(ServiceNotice.this, "앱이 강제로 종료되었습니다. 원활한 서비스를 위해 앱을 켜주세요", Toast.LENGTH_LONG).show();
    }
    else if(!SettingsActivity.select_noticeRefresh.isChecked() && isServiceRunning(".ServiceNotice")) {
            Log.d("exit","exitFree");
            SettingsActivity.select_noticeRefresh.setChecked(false);
            SettingsActivity.select_freeboardRefresh.setEnabled(true);
            SettingsActivity.select_allRefresh.setEnabled(true);
            onDestroy();
        } else if(SettingsActivity.select_noticeRefresh.isChecked() && !isServiceRunning(".ServiceNotice")){
            Toast.makeText(ServiceNotice.this, "♥", Toast.LENGTH_LONG).show();
        }
...
//                                            Log.d("refresh_er_old",""+NoldTITLE.get(0).toString());
                                            ...
                                            if (check == 0) {
                                                ...
                                                Intent intent2 = new Intent(ServiceNotice.this, MainNotice.class);
//                                                intent2.putExtra("plzUpdate", key);
                                                PendingIntent pendingIntent = PendingIntent.getActivity(ServiceNotice.this, 0, intent2, PendingIntent.FLAG_UPDATE_CURRENT);

                                                Notification.Builder builder = new Notification.Builder(ServiceNotice.this);
...
//    }
    ...
    private int getNotificationIcon() {
        ...
        return useWhiteIcon ? R.drawable.ic_stat_logo2 : R.drawable.ic_launcher; }
